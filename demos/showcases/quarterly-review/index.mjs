...
function addCover(pptx) {
	...
	slide.addText('KESTREL ANALYTICS', {
		x: 0.95,
		y: 2.55,
		w: 8,
		h: 0.35,
	...
	slide.addText(
		'Revenue up 13.7% on Platform strength — and the payback question\nwe need to settle before we sign off on Q4 hiring.',
		{
			x: 0.95,
			y: 4.85,
			w: 8.2,
...
function addAgenda(pptx) {
	...
	AGENDA.forEach((item, i) => {
		const y = top + i * rowH
		slide.addGroup([
			{ rect: { x: MARGIN, y, w: CONTENT_W, h: 0.012, fill: { color: BRAND.haze } } },
			{
				text: {
...
function addKpiBand(pptx) {
	...
	slide.addShape(ShapeType.rect, { x: MARGIN, y: 5.35, w: 0.08, h: 1.2, fill: { color: BRAND.clay } })
	slide.addText(
...
function addRevenueChart(pptx) {
	...
	slide.addChart(REVENUE_BY_SEGMENT, {
		...
		showLegend: true,
		legendPos: 'b',
		legendColor: BRAND.ash,
		legendFontSize: 11,
		showValue: true,
		dataLabelColor: BRAND.white,
		dataLabelFontSize: 10,
		dataLabelFormatCode: '$0.0',
		catAxisLabelColor: BRAND.ash,
		catAxisLabelFontSize: 11,
		catAxisLineShow: false,
		valAxisLabelColor: BRAND.ash,
		valAxisLabelFontSize: 11,
		valAxisLabelFormatCode: '$0"M"',
		valAxisLineShow: false,
		valGridLine: { color: BRAND.haze, size: 1, style: 'solid' },
	})
	...
	slide.addText(
		[
			...
			},
			{
				...
				options: { color: BRAND.ash, breakLine: true, fontSize: TYPE.body },
			},
			{ text: '\n', options: { fontSize: 8, breakLine: true } },
			{
				text: 'Licensing turned over.',
				options: { bold: true, color: BRAND.ink, breakLine: true, fontSize: TYPE.body },
			},
			{
				...
				options: { color: BRAND.ash, fontSize: TYPE.body },
			},
		],
		{
			x: noteX,
...
function addRevenueMix(pptx) {
	...
	slide.addText(
		[
			...
			{ text: 'total Q3 revenue', options: { fontSize: 11, color: BRAND.ash } },
		],
		{
			x: 2.35,
			y: 3.65,
	...
	slide.addNotes(
		'Platform crossing 55% of revenue is the number that changes how we should be valued. Say it out loud.'
	)
}
...
function addRegionTable(pptx) {
	...
	const bodyCell = (i) => ({
		fill: { color: i % 2 === 0 ? BRAND.white : BRAND.bone },
		color: BRAND.ink,
		fontSize: 13,
		fontFace: FONT.body,
		valign: 'middle',
		margin: [0.12, 0.16, 0.12, 0.16],
		border: [{ type: 'none' }, { type: 'none' }, { type: 'solid', color: BRAND.haze, width: 1 }, { type: 'none' }],
	})
	...
	const rows = REGIONS.map((r, i) => [
		...
		{ text: `$${r.revenue.toFixed(1)}M`, options: { ...bodyCell(i), align: 'right' } },
		{
			...
			options: { ...bodyCell(i), align: 'right', bold: true, color: r.growth >= 0 ? BRAND.brass : BRAND.rust },
		},
	...
	rows.push([
		...
		{ text: signedPct(13.7), options: { ...totalCell, align: 'right', color: BRAND.brass } },
	...
	slide.addText(
		'LATAM is the only region shrinking, on a base small enough that one churned account explains all of it. ' +
			'We are not reacting to it this quarter.',
		{
			x: MARGIN,
			y: 5.4,
			w: CONTENT_W,
...
function addRetentionChart(pptx) {
	...
	slide.addChart(RETENTION, {
		...
		h: 4.9,
		type: ChartType.line,
		chartColors: [BRAND.brass, BRAND.forest],
		lineSize: 3,
	...
	slide.addText(
		[
			{ text: '114%\n', options: { fontSize: 34, color: BRAND.brass, fontFace: FONT.head, breakLine: true } },
			{
				...
				options: { fontSize: TYPE.body, color: BRAND.pine },
			},
		],
		{
			x: calloutX,
			...
			rectRadius: 0.1,
			fill: { color: BRAND.brass, transparency: 92 },
			line: { color: BRAND.brass, width: 1 },
			fontFace: FONT.body,
			lineSpacingMultiple: 1.3,
		}
	)
	...
	slide.addNotes(
		'Gross retention at 96% is the one to watch — it is the number that would tell us the product is slipping.'
	)
}
...
function addRoadmap(pptx) {
	...
	ROADMAP.forEach((item, i) => {
		const accent = [BRAND.brass, BRAND.forest, BRAND.clay, BRAND.lichen][i]
		slide.addGroup([
			{
				shape: {
	...
	slide.addText(
		...
		],
		{
			...
			shape: ShapeType.roundRect,
			rectRadius: 0.08,
			fill: { color: BRAND.clay, transparency: 90 },
			fontFace: FONT.body,
			fontSize: TYPE.body,
			lineSpacingMultiple: 1.35,
		}
...
function addClosing(pptx) {
	...
	slide.addShape(ShapeType.rect, { x: 0.95, y: 3.95, w: 2.4, h: 0.06, fill: { color: BRAND.brass } })
	slide.addText(
		[
			{ text: 'Kestrel Analytics  ·  Office of the CFO\n', options: { color: BRAND.haze, breakLine: true } },
			{ text: 'Full data appendix and the Q3 model are in the board folder.', options: { color: BRAND.ash } },
		],
		{
			x: 0.95,
			y: 4.35,
...
export async function compose() {
	...
	pptx.title = 'Kestrel Analytics — Q3 FY26 Business Review'
	pptx.subject = 'Quarterly business review'
	pptx.author = 'Kestrel Analytics'
	pptx.company = 'Kestrel Analytics'

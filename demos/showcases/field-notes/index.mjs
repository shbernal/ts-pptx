...
function addCover(pptx) {
	...
	slide.addText(
		'Four transit systems, photographed between last train and first light — and what each one\nsays about the city that built it.',
		{
			x: 0.85,
			y: 5.15,
			w: 7,
...
function addAnnotatedMap(pptx) {
	...
	const notes = [
		...
		{
			n: '02',
			title: 'The loop as an anchor',
			body: 'Every other line is described by where it meets the Yamanote. The map is drawn outward from it.',
		},
		{
			n: '03',
			title: 'Letters before colours',
			body: 'Each line carries a letter and a number so the diagram survives being photocopied in grey.',
		},
	]
	const noteX = 8.6
	...
	slide.addNotes(
		'The duotone treatment here is a picture effect, not a pre-processed image — the source file is the plain map.'
	)
}
...
function addDimension(pptx) {
	...
	slide.addText(
		'A glTF binary embedded in the package: PowerPoint 2019 and later renders it live and lets you orbit it.',
		{
			x: 0.85,
			y: 1.35,
			w: 9,
	...
	slide.addNotes(
		'Click the model in slide show and drag — it orbits. In anything older than PowerPoint 2019 it is a still picture.'
	)
}

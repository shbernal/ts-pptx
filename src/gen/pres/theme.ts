...
export function makeXmlTheme(pres: PresentationPropsInternal): string {
	// Theme font faces are caller-supplied strings (`ThemeProps`) with no sanitizing upstream, so
	// they are escaped here: an unescaped `"` or `&` in a font name closes the attribute early and
	// emits non-parseable `theme1.xml`, which PowerPoint reports as a file needing repair.
	const majorFont = pres.theme?.headFontFace
		? `<a:latin typeface="${encodeXmlEntities(pres.theme?.headFontFace)}"/>`
		: '<a:latin typeface="Calibri Light" panose="020F0302020204030204"/>'
	const minorFont = pres.theme?.bodyFontFace
		? `<a:latin typeface="${encodeXmlEntities(pres.theme?.bodyFontFace)}"/>`
	...
	const majorEa = `<a:ea typeface="${encodeXmlEntities(pres.theme?.headFontFaceEA ?? '')}"/>`
	const minorEa = `<a:ea typeface="${encodeXmlEntities(pres.theme?.bodyFontFaceEA ?? '')}"/>`
	const majorCs = `<a:cs typeface="${encodeXmlEntities(pres.theme?.headFontFaceCS ?? '')}"/>`
	const minorCs = `<a:cs typeface="${encodeXmlEntities(pres.theme?.bodyFontFaceCS ?? '')}"/>`

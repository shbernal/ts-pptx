...
function makePlotAreaLayoutXml(rel: SlideRelChart): string {
	...
	const manualLayout = el('c:manualLayout', null, [
		...
		// flattening the chart emitters did not take it, and it stays. `prove-whitespace` freezes
		// intra-tag whitespace, and reported these when the flattening codemod first took them,
		// which is the whole reason it looks at them.

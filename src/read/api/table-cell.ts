/**
 * The `TableCell` read/edit proxy (`a:tc`), and the value types a cell's paint reads answer
 * with.
 *
 * Split out of `table.ts`, which held `Table`, `TableRow` and `TableCell` with no shared state
 * between them; this class alone was more than half the file. It stays whole here rather than
 * splitting again along its own properties/paint seam, because both halves read the same
 * `a:tcPr` and the style-graph fallback threads through both.
 */

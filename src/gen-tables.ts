 ...
 * this file re-exports it and retains the browser-only `tableToSlides()` DOM path.
 * `genTableToSlides` / `htmlBorderToProps` back the browser-only DOM flow, which is out
 * of active scope (see AGENTS.md) — the DOM-independent parts are factored into pure
 * helpers (`resolveHtmlColWidth`) that are unit-tested directly.
 */
...
import { getSlidesForTableRows } from './gen/table/autopage.js'

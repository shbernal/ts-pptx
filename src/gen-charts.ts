 ...
 * source PowerPoint opens when a user edits the chart. Reached from `gen-objects.ts`
 ...
 * presentation model.
 *
 * The chart's `chart.xml` DrawingML lives in `gen/chart/chart-xml.ts`; the
 * series↔worksheet-cell mapping the two sides share lives in `gen/chart/data-refs.ts`.
 */

import { ChartType, XML_DECL } from './core-enums.js'
import type { SlideRelChart, OptsChartDataInternal } from './core-interfaces.js'
import { dataLabels, dataValues, dataSizes, firstLabelGroup, getExcelColName } from './gen/chart/data-refs.js'

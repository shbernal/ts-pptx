...
- `docs/reference/api/` is generated by `pnpm run docs:api` from public package
  entry points and should not be hand-edited. `docs:api` also writes
  `docs/reference/api/sidebar.json`, which `docs/.vitepress/config.mts` serves as the
  sidebar under `/reference/api/`.

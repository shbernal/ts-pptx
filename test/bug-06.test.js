...
module.exports = [
	{
		...
		fn: async () => {
			...
			for (const id of distinctIds) {
				assert(definedIds.has(id), 'axId ' + id + ' referenced but not defined; defs:' + [...definedIds].join(',') + ' refs:' + [...distinctIds].join(','))
			}
			assert(catAx.length + valAx.length >= 4, 'expected at least 4 axis defs (primary+secondary), got ' + (catAx.length + valAx.length))
		}
	}
]

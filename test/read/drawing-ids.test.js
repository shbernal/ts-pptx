...
describe('drawing ids across carried and imported shapes', () => {
	...
	test('importShape keeps a grouped connector bound to the shape inside its group', async () => {
		...
		// the group, which the import does not carry, so only the bindings inside it are compared.
		const source = await openFixture('mixed')
		const sourceGroup = source.slides[4].shapes[4].element_
		const before = bindingTargets(sourceGroup)
		assertEqual(
			before.filter((name) => name !== null).length,
			2,
			'the fixture group binds two connectors inside itself'
		)
		...
		const after = bindingTargets(imported.element_)
		before.forEach((name, i) => {
			if (name !== null) assertEqual(after[i], name, `binding ${i} still names ${name}`)
		})
	})
})

...
module.exports = [
	...
	},
	{
		...
		fn: async () => {
			...
			assert(shadow.blur === 6,
				'expected shadow.blur to remain 6 (pt); got ' + shadow.blur)
			assert(shadow.offset === 2,
				'expected shadow.offset to remain 2 (pt); got ' + shadow.offset)
			assert(shadow.opacity === 0.15,
				'expected shadow.opacity to remain 0.15; got ' + shadow.opacity)
			assert(shadow.angle === undefined,
				'expected shadow.angle to remain undefined; got ' + shadow.angle)
			assert(shadow.color === '000000',
				'expected shadow.color to remain "000000"; got ' + shadow.color)
		}
	},
	{
		...
		fn: async () => {
			...
			assert(shadow.blur === 6,
				'expected image-branch shadow.blur to remain 6 (pt); got ' + shadow.blur)
			assert(shadow.opacity === 0.15,
	...
	},
	{
		...
		fn: async () => {
			...
			assert(xml.indexOf('<a:effectLst>') !== -1,
				'expected <a:effectLst> in single-write output')
			// blurRad=valToPts(6)=76200; dist=valToPts(2)=25400; dir=Math.round(270*60000)=16200000; alpha=Math.round(0.15*100000)=15000
			assert(xml.indexOf('blurRad="76200"') !== -1,
				'expected blurRad="76200"; got: ' + xml)
			assert(xml.indexOf('dist="25400"') !== -1,
				'expected dist="25400"; got: ' + xml)
			assert(xml.indexOf('dir="16200000"') !== -1,
				'expected dir="16200000"; got: ' + xml)
			assert(xml.indexOf('<a:alpha val="15000"/>') !== -1,
				'expected <a:alpha val="15000"/>; got: ' + xml)
			// Belt-and-braces: no absurdly large EMU values that signal double-conversion
			assert(xml.indexOf('blurRad="967740000"') === -1,
				'unexpected double-converted blurRad in output: ' + xml)
		}
	}
]

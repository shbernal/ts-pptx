...
export default [
	...
	},
	{
		...
		fn: async () => {
			...
			assert(
				/Extension="jpg"/.test(xml) || /Extension="jpeg"/.test(xml),
	...
	},
	{
		...
		fn: async () => {
			...
			assert(
				/PartName="\/ppt\/slideMasters\/slideMaster1\.xml"/.test(xml),
				'expected slideMaster1.xml Override; got: ' + xml
			)
			...
			assert(
				/PartName="\/ppt\/slideLayouts\/slideLayout1\.xml"/.test(xml),
				'expected slideLayout1.xml Override; got: ' + xml
			)
			assert(
				/PartName="\/ppt\/notesMasters\/notesMaster1\.xml"/.test(xml),
				'expected notesMaster1.xml Override; got: ' + xml
			)
			assert(
				/PartName="\/ppt\/notesSlides\/notesSlide1\.xml"/.test(xml),
				'expected notesSlide1.xml Override; got: ' + xml
			)
		},
	},
]

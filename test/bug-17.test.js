...
const { build, listEntries, assert } = require('./helpers')
...
function chartsOrEmbeddingsEntries(zip) {
	return listEntries(zip).filter(p => p.startsWith('ppt/charts/') || p === 'ppt/charts' || p.startsWith('ppt/embeddings/') || p === 'ppt/embeddings')
}
...
module.exports = [
	{
		...
		fn: async () => {
			...
			assert(stray.length === 0, 'expected no charts/embeddings entries for chart-free deck; got: ' + JSON.stringify(stray))
		}
	},
	{
		...
		fn: async () => {
			...
			const stray = chartsOrEmbeddingsEntries(zip)
			assert(stray.length === 0, 'expected no charts/embeddings entries for image-only deck; got: ' + JSON.stringify(stray))
		}
	},
	{
		...
		fn: async () => {
			...
			assert(chartEntries.length > 0, 'expected ppt/charts/ entries when chart present; got: ' + JSON.stringify(entries))
			assert(embedEntries.length > 0, 'expected ppt/embeddings/ entries when chart present; got: ' + JSON.stringify(entries))
		}
	}
]

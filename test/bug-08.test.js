...
module.exports = [
	{
		...
		fn: async () => {
			...
			assert(xml.indexOf('val="00000020"') === -1,
				'expected no 8-char val="00000020" in slide XML; got:\n' + xml)
			const expectedAlpha = alphaPct('20') // 12549
			const expectedFragment = `<a:srgbClr val="000000"><a:alpha val="${expectedAlpha}"/></a:srgbClr>`
			assert(xml.indexOf(expectedFragment) !== -1,
				`expected ${expectedFragment} in slide XML; got:\n` + xml)
		}
	},
	{
		...
		fn: async () => {
			...
			const xml = await buildSlide1(pres)
			assert(xml.indexOf('val="00FF0080"') === -1,
				'expected no 8-char val="00FF0080" in slide XML; got:\n' + xml)
	...
	},
	{
		...
		fn: async () => {
			...
			const xml = await buildSlide1(pres)
			assert(xml.indexOf('val="0000FF40"') === -1,
				'expected no 8-char val="0000FF40" in slide XML; got:\n' + xml)
	...
	},
	{
		...
		fn: async () => {
			...
			assert(effectBlock.indexOf('<a:srgbClr val="000000">') !== -1,
				'expected <a:srgbClr val="000000"> inside shadow effectLst; got:\n' + effectBlock)
			// Derived opacity: 32/255 ≈ 0.12549 → ×100000 ≈ 12549
			const expectedAlpha = alphaPct('20') // 12549
			assert(effectBlock.indexOf(`<a:alpha val="${expectedAlpha}"/>`) !== -1,
	...
	},
	{
		...
		fn: async () => {
			...
			const xml = await buildSlide1(pres)
			assert(xml.indexOf('val="88888880"') === -1,
				'expected no 8-char val="88888880" in slide XML; got:\n' + xml)
	...
	},
	{
		...
		fn: async () => {
			...
			assert(xml.indexOf('val="FF0000FF"') === -1,
				'expected no 8-char val="FF0000FF" in slide XML; got:\n' + xml)

...
| `comparison/measure.mjs` | Generator | Builds every probe with ts-pptx and with upstream pptxgenjs, reads the emitted parts, measures the other three families, and writes `comparison/snapshot.json` | manual (release step, see `docs/contributing/releasing.md`) |
...
| `comparison/render.mjs` | Generator + Gate | Renders `comparison/snapshot.json` into `docs/comparison.md`, `docs/comparison-method.md`, `docs/comparison-syntax.md` and the generated region of `README.md`, with the group labels from `comparison/groups.json`; `--check` re-renders in memory and fails on drift | manual (`comparison:render`); `--check` in `check:core` (`comparison:check`) |

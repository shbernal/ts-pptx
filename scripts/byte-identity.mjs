 ...
 * whitespace only where it provably cannot be content. It is for a change planned as
 * whitespace-only before it was made; `docs/contributing/testing.md` ("Proving a change is
 * whitespace-only") has the procedure. `check` remains the gate for every other refactor.
 */
...
\`prove-whitespace\` is for a change planned as whitespace-only (docs/contributing/testing.md),

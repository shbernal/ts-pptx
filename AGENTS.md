...
- **`lint:chars`.** Pre-commit scans the staged content of what you are committing and
  ...
  purpose: see the em-dash ratchet below.
...
- `pnpm run lint:chars` (part of `check:static`, and of both git hooks) keeps **em dashes** out of what a reader sees: the README, `www/`, and prose in `docs/`. Config is `charcheck.config.js`, and the rules are scoped, so a dash in a code fence, a comment or a stylesheet is not a finding. It is **two gates in one**. Anything already clean errors on a new dash, and that is the whole of the site and the README. `docs/**/*.md` arrived with 684 of them, far too many to rewrite as a side effect of adding the gate, so those warn instead and `--max-warnings 684` in the script freezes the total: it may shrink, never grow. To retire a page from the backlog, clean it with `node node_modules/charcheck/dist/cli.js --fix <path>` (a positional path is *intersected* with each rule's `include`, never substituted for it, so it cannot widen the scan), add it to `DOCS_CLEAN` in the config, and lower the number by what you removed, in the same commit. `pnpm run lint:chars:fix` is the same thing repo-wide. **Read that diff.** The fixer guesses at prose and says so on every run. The two guesses that were wrong (shbernal/charcheck#11, #12) are fixed as of charcheck 0.2.2, verified against this tree: a dash opening a hard-wrapped line no longer moves its replacement to the start of the next line, and a colon later in the sentence no longer downgrades the dash to a comma splice. Reading the diff is still the rule, because a fix is still a guess. No hook passes `--fix`, deliberately: rewriting a sentence on its way into a commit is not a guess a hook should make unsupervised. If the gate ever seems *wrong* rather than inconvenient — it passes a file you know holds a dash, flags a region its scope says it does not read, or tempts you to add an `exclude` to make a finding go away — load the `charcheck-upstream` skill in `.agents/skills/` and file it instead. That failure is silent by construction: a rule whose globs reach nothing exits 0 and looks exactly like a clean run, which is why `node node_modules/charcheck/dist/cli.js --report-issue` exists. It prints each rule as it *resolved*, with the file count each one matched, and a count of zero is usually the whole bug.

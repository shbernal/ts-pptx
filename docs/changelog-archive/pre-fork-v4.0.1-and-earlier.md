# Changelog archive — pre-fork history (v4.0.1 and earlier)

This is the upstream [`gitbrent/PptxGenJS`](https://github.com/gitbrent/PptxGenJS)
changelog history, from **v4.0.1 (2025-06-25)** back to the initial **v1.0.0
(2016-03-29)** release — everything *before* this fork's first release
(v5.0.0). It is split out of the live [`CHANGELOG.md`](../../CHANGELOG.md) to
keep that file focused on the fork's own majors (v5.0.0 onward) and fast to load
for humans and agents. Nothing here is edited; it is the original entries
verbatim.
